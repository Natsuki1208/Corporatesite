// @ts-nocheck
import { visionTiming } from './vision-animation';
import { topologyTiming } from './problem-topology';
import { eventJourneyContract } from './event-journey';
import { lifecycleContract } from './lifecycle-animation';
import { navigationSelectors } from './navigation';
import { solutionKeys } from './solution-explorer';
import { readinessContract } from './readiness-console';
(() => {
  'use strict';
  ...
  const menuButton = document.querySelector(navigationSelectors[0]);
  const nav = document.querySelector(navigationSelectors[1]);
  ...
  const visionMatrixStages = [...document.querySelectorAll('[data-matrix-row]')];
  ...
  function renderVisionLanguage() {
    ...
    else if (visionState.running && visionState.stage < 0) visionStatus.textContent = language === 'en' ? 'Aligning distributed signals' : '正在對齊分散訊號';
  ...
  function completeVision() {
    ...
    visionStages.forEach((stage) => { stage.classList.remove('active'); stage.classList.add('complete'); });
    visionMatrixStages.forEach((stage) => { stage.classList.remove('active'); stage.classList.add('complete'); });
  ...
  function runVision() {
    ...
    visionSystem.classList.remove('complete','running');
    visionStages.forEach((stage) => stage.classList.remove('active','complete'));
    visionMatrixStages.forEach((stage) => stage.classList.remove('active','complete'));
    if (reduceMotion.matches) { completeVision(); return; }
    void visionSystem.offsetWidth;
    visionSystem.classList.add('running');
    renderVisionLanguage();
    visionStages.forEach((stage, index) => {
      visionState.timers.push(window.setTimeout(() => {
        ...
        visionMatrixStages.forEach((item, itemIndex) => {
          const stageIndex = itemIndex % 5;
          item.classList.toggle('active', stageIndex === index);
          if (stageIndex < index) item.classList.add('complete');
        });
        renderVisionLanguage();
      }, visionTiming.initialDelay + index * visionTiming.stageDelay));
    });
    visionState.timers.push(window.setTimeout(completeVision, visionTiming.completeAt));
  }
  document.querySelector('[data-vision-replay]').addEventListener('click', runVision);
  let visionInView = false;
  const updateVisionHeartbeat = () => visionSystem.classList.toggle('heartbeat-active', visionInView && !document.hidden && !reduceMotion.matches);
  ...
  function runProblemDemo() {
    ...
    }, topologyTiming.initialDelay + index * topologyTiming.nodeDelay)));
    problemDemo.timers.push(window.setTimeout(finishProblemDemo, topologyTiming.completeAt));
  }
  ...
  function renderAiChange(index) {
    aiChangeIndex = index;
    const [kicker,title,copy,word] = aiChangeContent[index];
    aiChangeStage.dataset.activeChange = String(index);
    aiChangeStage.querySelector('[data-ai-change-kicker]').textContent = kicker;
    aiChangeStage.querySelector('[data-ai-change-title]').textContent = title;
    aiChangeStage.querySelector('[data-ai-change-copy]').textContent = copy;
  ...
  aiChangeTabs.forEach((tab,index) => {
    ...
    tab.addEventListener('keydown', (event) => {
      ...
      event.preventDefault();
      let target = index;
      if (event.key === 'ArrowLeft') target = (index - 1 + aiChangeTabs.length) % aiChangeTabs.length;
  ...
  reduceMotion.addEventListener('change', () => {
    updateVisionHeartbeat();
    if (reduceMotion.matches) {
      if (visionState.running) completeVision();
      renderAiChange(3);
    }
    scheduleAiChange();
  });
  ...
  if (Object.keys(readinessData).length !== readinessContract.categories) {
    throw new Error('Readiness category contract mismatch');
  }
