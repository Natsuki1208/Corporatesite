(() => {
  ...
  const table = {
    ...
    '讓訊號清楚可讀，':['Make signals understandable,','シグナルを分かりやすく、','신호는 명확하게,'],
    '讓行動保有控制。':['while keeping action controlled.','行動には制御を。','실행은 통제합니다.'],
    ...
    '看見服務關係，':['See service relationships,','サービスの関係を見て、','서비스 관계를 파악하고,'],
    '不只是監控圖表。':['not just monitoring charts.','監視チャートを越える。','모니터링 차트 그 이상을 봅니다.'],
    ...
    '營運洞察 · CONCEPT / DEMO':['Operations Insights · CONCEPT / DEMO','運用インサイト · CONCEPT / DEMO','운영 인사이트 · CONCEPT / DEMO'],
    '讓每一次事件，':['Turn every incident','すべてのイベントを、','모든 이벤트를'],
    '成為更快決策基礎。':['into a foundation for faster decisions.','より速い判断の基盤へ。','더 빠른 결정의 기반으로 만듭니다.'],
    '將任務紀錄與團隊註記整理成可回顧、可持續改善的營運知識。':['Organize mission records and team notes into reviewable operations knowledge for continuous improvement.','ミッション記録とチームの注記を、振り返りと継続改善に使える運用知識へ整理します。','임무 기록과 팀 메모를 검토하고 지속적으로 개선할 수 있는 운영 지식으로 정리합니다.'],
  ...
  Object.assign(table, {
    ...
    'Classifier：事件分類':['Classifier: Event classification','Classifier：イベント分類','Classifier: 이벤트 분류'],
    'Local AI：代碼化脈絡處理':['Local AI: Coded context processing','Local AI：コード化文脈処理','Local AI: 코드화 맥락 처리'],
    'Reasoning：產生檢查建議':['Reasoning: Generate check recommendations','Reasoning：確認提案を生成','Reasoning: 점검 제안 생성'],
    'Rule Engine：限制允許範圍':['Rule Engine: Limit allowed scope','Rule Engine：許可範囲を制限','Rule Engine: 허용 범위 제한'],
    'Human Gate：最後決定':['Human Gate: Final decision','Human Gate：最終決定','Human Gate: 최종 결정'],
    'COMMAND：多個模擬訊號收斂成單一任務線。':['COMMAND: Demo signals converge into one mission path.','COMMAND：模擬シグナルを一つのミッション経路へ集約。','COMMAND: 데모 신호를 하나의 임무 흐름으로 통합합니다.'],
    'OBSERVE：主機、網路與服務節點依序建立關聯。':['OBSERVE: Hosts, networks, and services connect in sequence.','OBSERVE：ホスト、ネットワーク、サービスを順に関連付け。','OBSERVE: 호스트, 네트워크, 서비스 노드를 순서대로 연결합니다.'],
    'DEFEND：異常節點經掃描後被清楚標記。':['DEFEND: Scanning clearly marks the anomalous node.','DEFEND：スキャンで異常ノードを明確に表示。','DEFEND: 스캔 후 이상 노드를 명확히 표시합니다.'],
    'CONTROL：工作流停在金色人工核准閘門。':['CONTROL: The workflow stops at the human approval gate.','CONTROL：ワークフローは人の承認ゲートで停止。','CONTROL: 워크플로는 사람 승인 게이트에서 멈춥니다.'],
    'CONNECT：機房、雲端與據點只交換狀態摘要。':['CONNECT: Sites and cloud exchange status summaries only.','CONNECT：拠点とクラウドは状態概要のみ交換。','CONNECT: 시설과 클라우드는 상태 요약만 교환합니다.'],
    'LEARN：事件、建議與人工決定進入知識循環。':['LEARN: Events, recommendations, and human decisions enter the knowledge loop.','LEARN：イベント、提案、人の判断を知識ループへ。','LEARN: 이벤트, 제안, 사람의 결정을 지식 순환에 반영합니다.'],
    '智慧製造：彙整產線與服務告警，AI 提供檢查建議，設備操作仍由現場人員確認。':['Smart manufacturing: AI organizes production and service alerts and suggests checks; on-site staff confirm equipment actions.','スマート製造：生産・サービスアラートを整理し、AI が確認を提案。機器操作は現場が承認します。','스마트 제조: 생산 및 서비스 알림을 정리하고 AI가 점검을 제안하며 장비 작업은 현장 담당자가 확인합니다.'],
    '資料中心：整合主機、虛擬化與網路狀態，只讀取核准範圍。':['Data centers: Unify host, virtualization, and network status within approved read-only scope.','データセンター：ホスト、仮想化、ネットワーク状態を承認済みの読み取り範囲で統合します。','데이터센터: 호스트, 가상화, 네트워크 상태를 승인된 읽기 전용 범위에서 통합합니다.'],
    '醫療營運：先遮蔽識別資料，再整理系統可用性事件；不接觸醫療診斷。':['Healthcare operations: Mask identifiers before organizing availability events; no medical diagnosis is involved.','医療運用：識別情報をマスクして可用性イベントを整理し、医療診断は扱いません。','의료 운영: 식별 정보를 마스킹한 뒤 가용성 이벤트를 정리하며 의료 진단은 다루지 않습니다.'],
    '物流與據點：將分散據點狀態整理成摘要，不執行遠端控制。':['Logistics and sites: Summarize distributed site status without remote control.','物流と拠点：分散拠点の状態を要約し、遠隔操作は行いません。','물류 및 지점: 분산된 지점 상태를 요약하며 원격 제어는 실행하지 않습니다.'],
    '企業 IT：協助 MIS 排序事件、建立紀錄並保留人工決策。':['Enterprise IT: Help MIS prioritize incidents, keep records, and retain human decisions.','企業 IT：MIS の優先順位付けと記録を支援し、人の判断を保持します。','기업 IT: MIS의 이벤트 우선순위 지정과 기록을 돕고 사람의 결정을 유지합니다.'],
    'CONCEPT PREVIEW：此文章／活動為虛構展示，目前沒有對外發布或報名連結。':['CONCEPT PREVIEW: This article or event is fictional and has no public publication or registration link.','CONCEPT PREVIEW：この記事／イベントは架空で、公開・登録リンクはありません。','CONCEPT PREVIEW: 이 글 또는 이벤트는 가상이며 공개 게시나 등록 링크가 없습니다.'],
    '展示模式：訂閱功能尚未連接，沒有儲存資料或送出網路 Request。':['Demo mode: Subscription is not connected; no data is stored or sent.','デモモード：購読機能は未接続で、データの保存・送信は行いません。','데모 모드: 구독 기능은 연결되지 않았으며 데이터를 저장하거나 전송하지 않습니다.'],
    'LAB 為 Elias Net 概念頻道展示，尚未連接外部平台。':['LAB is an Elias Net concept channel demo and is not connected to an external platform.','LAB は Elias Net のコンセプトチャンネル表示で、外部サービスには接続していません。','LAB은 Elias Net 개념 채널 데모이며 외부 플랫폼에 연결되지 않았습니다.'],
    'MEDIA 為 Elias Net 概念頻道展示，尚未連接外部平台。':['MEDIA is an Elias Net concept channel demo and is not connected to an external platform.','MEDIA は Elias Net のコンセプトチャンネル表示で、外部サービスには接続していません。','MEDIA는 Elias Net 개념 채널 데모이며 외부 플랫폼에 연결되지 않았습니다.'],
    'COMM 為 Elias Net 概念頻道展示，尚未連接外部平台。':['COMM is an Elias Net concept channel demo and is not connected to an external platform.','COMM は Elias Net のコンセプトチャンネル表示で、外部サービスには接続していません。','COMM은 Elias Net 개념 채널 데모이며 외부 플랫폼에 연결되지 않았습니다.'],
    '流程已暫停，確認並繼續展示按鈕現在可用。':['The flow is paused. Confirm and Continue Demo is now available.','フローは一時停止中です。「確認してデモを続行」が利用できます。','흐름이 일시정지되었습니다. 이제 확인 후 데모 계속 버튼을 사용할 수 있습니다.'],
    '展示完成，Actual Action 仍為 0。':['Demo complete. Actual Action remains 0.','デモ完了。Actual Action は 0 のままです。','데모 완료. Actual Action은 계속 0입니다.'],
    '自動播放已暫停。':['Autoplay paused.','自動再生を一時停止しました。','자동 재생이 일시정지되었습니다.'],
    '必須先完成 MIS 人工確認，才能進入展示完成。':['MIS confirmation is required before the demo can complete.','デモ完了には MIS の確認が必要です。','데모를 완료하려면 먼저 MIS 확인이 필요합니다.'],
