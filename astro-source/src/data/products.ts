...
export const products: ProductDefinition[] = [
  {
    ...
    headline:L('找到答案，也看見它從何而來。','Find the answer. See where it came from.'),
    intro:L('協助團隊搜尋公司文件、整理回答並附上來源，方便回到原文核對。','Help teams search company documents and prepare sourced answers they can check against the originals.'),
  ...
  },
  {
    ...
    headline:L('先理解需求，再讓每一次回應更貼近人心。','Understand the need, then respond with greater care.'),
    intro:L('整理對話、案件紀錄與服務規範，協助準備回覆草稿。由客服確認內容，再回應客戶。','Bring conversations, case history and service policies together to prepare a draft. Service agents review the content before replying.'),
  ...
  },
  {
    ...
    headline:L('讓流程清楚推進，讓每次協作自然銜接。','Clear progress. Natural collaboration.'),
    ...
    scenario:L('資料進入工作流程後，AI 整理重點、建立審核任務並提示下一步；由負責人確認後，才準備更新系統並留下紀錄。','When information enters the workflow, AI organizes key details, creates a review task and prepares the next step. The system update is prepared only after the responsible person confirms it, with a record retained.'),
  ...
  },
  {
    ...
    headline:L('先看清影響，再從容決定下一步。','See the impact clearly, then choose the next step.'),
    ...
    capabilities:[L('集中系統與網路告警','Bring system and network alerts together'),L('找出相關異常','Connect related issues'),L('整理事件經過','Build a clear incident timeline'),L('標出影響範圍','Show affected services'),L('準備檢查順序','Prepare the next checks')],
    scenario:L('系統與網路同時出現多項告警。AI 合併重複訊息、整理影響範圍；工程師決定觀察、轉派或進一步處理。','System and network alerts appear at the same time. AI groups duplicate signals and shows the affected services; an engineer decides whether to observe, hand off or take further action.'),
    collaboration:L('AI 負責整理告警與線索；工程師判斷原因、優先順序與實際操作。','AI organizes alerts and clues. Engineers decide the cause, priority and any real action.'),
    dataPermissions:L('需要監控資料、系統日誌、事件單與服務關係。查詢預設唯讀，任何操作都要另外授權。','Requires monitoring data, system logs, incident tickets and service relationships. Queries are read-only by default, and every action requires separate authorization.'),
  ...
  },
  {
    ...
    headline:L('讓恰到好處的協助，自然走進日常。','Let the right help become a natural part of daily life.'),
    intro:L('從拿取物品到簡單移動協助，Elias Home 先確認需求再行動。使用者保有選擇，也能隨時停止。','From fetching items to simple mobility support, Elias Home confirms the request before acting. Users keep the choice and can stop it at any time.'),
  ...
  },
  {
    ...
    headline:L('讓機器先行，讓人更安全地前進。','Let the robot go first, so people can move forward more safely.'),
    intro:L('面對倒塌、有毒或難以進入的現場，Elias Rescue 先回傳影像與環境資訊。救援團隊掌握風險後，再指揮行動。','In collapsed, toxic or hard-to-reach environments, Elias Rescue relays video and conditions first. Responders understand the risk, then direct the mission.'),
