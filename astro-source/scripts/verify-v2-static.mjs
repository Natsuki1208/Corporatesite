...
for(const route of routes){
 ...
 const canvases=(html.match(/<canvas/g)??[]).length;const allowsVideo=home||products.includes(productRoute);mark(`${route}:motion-budget`,(!html.includes('<video')||allowsVideo)&&canvases<=(home?1:0),`canvas=${canvases}`);
 if(home){mark(`${route}:scenes`,(html.match(/<section[^>]+data-motion-scene/g)??[]).length===5);mark(`${route}:service-portfolio`,(html.match(/class="v2-architecture-families"/g)??[]).length===1&&html.includes('/digital-intelligence/')&&html.includes('id="service-software"')&&html.includes('id="service-data"')&&html.includes('id="service-digital-assets"'));mark(`${route}:capabilities`,(html.match(/data-motion-reveal/g)??[]).length>=8&&html.includes('id="features"')&&html.includes('id="feature-understand"')&&!html.includes('APPLICATION AREAS'));mark(`${route}:no-duplicate-product-list`,!html.includes('class="v2-product-row'));mark(`${route}:hero-media`,html.includes('/media/elias-net-home-hero.mp4')&&html.includes('/images/v2/elias-net-home-hero-desktop.jpg'));mark(`${route}:hero-message`,html.includes('The future of yesterday, today.')&&!html.includes('aria-label="SENSE, COORDINATE, AUTHORIZE, RESPOND"')&&!html.includes('data-hero-replay'));mark(`${route}:logo-core`,html.includes('class="v2-brand-mark')&&html.includes('mark-module module-nw')&&html.includes('mark-module module-se')&&html.includes('mark-core-frame'));}
 if(productRoute){
