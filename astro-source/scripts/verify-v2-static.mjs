...
for(const route of routes){
 ...
 if(enterpriseRoute){mark(`${route}:enterprise-service`,html.includes(`data-page="enterprise-service"`)&&html.includes(`/media/${enterpriseRoute}-service.mp4`)&&html.includes(`/images/v2/services/${enterpriseRoute}-service.jpg`)&&(html.match(/<video/g)??[]).length===1);}
 if(productRoute){
  ...
  if(liveProduct)mark(`${route}:hero-video`,html.includes(`/media/${productRoute}-live-scenario.mp4`)&&html.includes('class="v2-product-hero-video"')&&html.includes(' muted')&&html.includes(' loop')&&html.includes(' playsinline')&&videoCount===1&&!html.includes('data-product-video'),`video=${videoCount}`);
