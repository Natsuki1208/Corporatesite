...
for(const route of routes){
 ...
 if(home){mark(`${route}:scenes`,(html.match(/<section[^>]+data-motion-scene/g)??[]).length===6);mark(`${route}:products`,(html.match(/class="v2-product-row/g)??[]).length===6&&html.includes('Elias Home')&&html.includes('Elias Rescue'));mark(`${route}:hero-media`,html.includes('/images/v2/elias-net-home-hero-desktop.jpg')&&html.includes('/images/v2/elias-net-home-hero-mobile.jpg'));mark(`${route}:hero-message`,html.includes('The future of yesterday, today.')&&html.includes('aria-label="SENSE, COORDINATE, AUTHORIZE, RESPOND"')&&html.includes('data-hero-replay'));mark(`${route}:logo-core`,html.includes('class="v2-brand-mark')&&html.includes('mark-module module-nw')&&html.includes('mark-module module-se')&&html.includes('mark-core-frame'));for(const [slug,fileName] of Object.entries(productHero))mark(`${route}:product-media:${slug}`,html.includes(`/images/v2/products/${fileName}`));}
 ...
 if(route.endsWith('company/index.html')){mark(`${route}:company-v2`,html.includes('class="v2-company"')&&html.includes('v2-company-hero-grid'));mark(`${route}:company-order`,html.indexOf('company-mission')<html.indexOf('company-governance')&&html.indexOf('company-governance')<html.indexOf('company-solutions')&&html.indexOf('company-solutions')<html.indexOf('company-organization')&&html.indexOf('company-organization')<html.indexOf('company-start'));}
