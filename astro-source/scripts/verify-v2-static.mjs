...
for(const route of routes){
 ...
 const canvases=(html.match(/<canvas/g)??[]).length;const allowsVideo=home||['elias-home','elias-rescue','elias-care','elias-ops'].includes(productRoute);mark(`${route}:motion-budget`,(!html.includes('<video')||allowsVideo)&&canvases<=(home?1:0),`canvas=${canvases}`);
 ...
 if(productRoute){
  const liveProduct=['elias-home','elias-rescue','elias-care','elias-ops'].includes(productRoute);const videoCount=(html.match(/<video/g)??[]).length;
